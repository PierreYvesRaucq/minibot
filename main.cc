...
int main()
{
	...
	int quit = 0; // variable de sortie d'urgence
	...
	while(1){
		...
		fwd(D, goal, structure); // Pierre : fonction qui renvoie (ou actualise un paramêtre de la structure on verra après) les 2 commande à donner au roues pour que le robot avance d'une dist D

		//on va rechrcher le paramètre commande des roue dans la structure et on les applique
		double cmdL = structure->theCtrlOut->wheel_commands[0];
		double cmdR = structure->theCtrlOut->wheel_commands[1];
